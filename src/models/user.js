...
class User{
    ...
    static async getUserByEmail(email){
        //get first user from database that matches id
        const result = (await query.query("SSELECT u.*, e.access_level FROM Users AS u INNER JOIN employee_access AS e ON u.id = e.user_id WHERE u.email = @email", {"email": email})).recordset[0]
        //return null if no user found
        return result ? result : null
    }
